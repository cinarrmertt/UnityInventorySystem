...
namespace MTProject.PlayerController
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        public void FixedUpdate()
        {
            ...
            Ray ray = new Ray(_playerCamera.transform.position,_playerCamera.transform.TransformDirection(Vector3.forward));
            RaycastHit hit;
            float maxDistance = 5f;
            
            if (Physics.Raycast(ray,out hit,maxDistance,layerMask))
            {
                Debug.Log("etkileşime girebilir");
                Debug.DrawRay(ray.origin,ray.direction*hit.distance,Color.green);
                if (hit.collider.gameObject.tag=="Item"&& Input.GetKey(KeyCode.E))
                {
                    if (_inventoryManager.AddItem(hit.collider.gameObject.GetComponent<Item>().item))
                    {
